...
## 🟧 데스매치 3모드 분리 (S2, 2026-06-25) — 유저 요청
진입 시 **모드 선택 화면**(메뉴 `dmmode`) → 카트선택 → 매치. `createArenaWorld(defs, modeKey)`, `DM_MODES`:
- **점수전(score)**: 5분 점수전 · 무한 리스폰 · 맵 고정 **넓음(R82)** · 최고점 승
- **서바이벌(survival)**: 목숨 1 · 리스폰X · 맵 **축소** · 최후의 1인
- **목숨3개(lives)**: 목숨 3 · 죽으면 −1목숨+리스폰 · 맵 **축소** · 0되면 탈락 · 최후의 1인
- **점프대 랜덤 위치**(매 게임 `placePads()` 재배치, 간격 확보)
- HUD: 점수전=`TIME m:ss`, 서바이벌/목숨=`생존 N`; 순위판에 ♥목숨/💀탈락. 온라인은 start 메시지로 mode 동기화
- 검증 `dm_survival_hud`(생존2·💀·음수점수)·`dm_lives_hud`(♥♥♥·생존7), 에러0

## 🟧 점수제 5분 매치 전환 (S1, 2026-06-25) — 유저 요청
서바이벌/즉사 → **점수제 5분 프래그 매치**:
- 죽음(트레일·경계·윌리전복) **−1** + **2초 리스폰**(안전지점, 무적 2.2s) · 탈락 없음
- 킬(내 트레일에 적 충돌, 또는 보호막/super로 들이받기) **+2**
- 아레나 **고정**(축소 없음, R=60) · **5분 타이머** 종료 시 최고점 승
- **순위 기반 아이템**(20초마다 자동, 1슬롯, **F** 발동): 1위 ⤴️점프 · 상위 💨부스트 · 하위 🛡️보호막(3s 무적) · 꼴찌 🔥무적질주(4s 무적+가속)
- HUD: 좌상단 순위판 · 중앙 TIME m:ss · 하단 아이템 슬롯. 봇도 아이템 사용. 온라인 점수/킬/리스폰 동기화(2-PC 실테스트 필요)
- 검증 `dm_score_hud`: 에러0·타이머·리스폰·순위별 아이템(jump~super)·F 발동(boost+shield)
